 ...
 */
package com.cif.utils.baseutil;
...
public final class INIFile {
    ...
    public String getStringProperty(String pstrSection, String pstrProp) {
        ...
        objSec = mhmapSections.get(pstrSection);
        if (objSec != null) {
     ...
     * specified section.
     *
     * @param pstrSection the name of the section for which names of properties
     * is to be retrieved.
    ...
    /**
     * Flush changes back to the disk file. If the disk file does not exists
    ...
    public boolean save() {
        ...
        try {
            if (mhmapSections.size() == 0) {
                return false;
            }
            //创建目录
            File dir = new File(PathUtil.getFullPathNoEndSeparator(mstrFile));
            if (!dir.exists()) {
                dir.mkdirs();
            }
            objFile = new File(mstrFile);
            if (objFile.exists()) {
                objFile.delete();
            }
            objWriter = new FileWriter(objFile);
            itrSec = mhmapSections.keySet().iterator();
            while (itrSec.hasNext()) {
                strName = itrSec.next();
                objSec = mhmapSections.get(strName);
    ...
    private Properties getEnvVars() {
        ...
        try {
            ...
            // Windows 7 64位环境 出现过程序卡死在 br.readLine() 所以添加此判断
            // p.getErrorStream():"      0 [main] us 0 init_cheap: VirtualAlloc pointer is null, Win32 error 487"
    ...
    /**
     * Reads the INI file and load its contentens into a section collection
     * after parsing the file line by line.
     */
...
//
        objINI.addSection("Folders", "Directories where generated files are stored");
        objINI.setStringProperty("Folders", "folder1", "c:\\Temp", null);
        objINI.setStringProperty("Folders", "folder2", "c:\\Temp\\Backup", null);
    ...
    private class INISection {
         ...
         *
         * @param pstrProp the property whose underlying value is to be
         * etrieved.
